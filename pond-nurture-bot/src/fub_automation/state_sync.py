...
The six workflows in .github/workflows each pull this file at the start of a job
and push it back at the end. Before #9 the push was an unconditional whole-file
overwrite: no compare-and-swap, no merge. Any workflow that pulled before a
sibling's push and finished after it silently threw away everything the sibling
had written, and both pushes reported success. On 2026-08-11 that happened twice
in one day — Daily Automation's 150 pond sends, then the repair rebuilding them,
clobbered by Reply Detection #459 four minutes after the backfill reported
success. Nothing in the system noticed either time.

Cadences guarantee the overlap and cannot be serialised away: Speed-to-Lead runs
every five minutes and is the 30/60-minute new-lead promise, so it cannot be
parked behind a shared concurrency group for the two hours the daily run may
hold it (candidate fix 1 in #9, rejected there and here).
...
working tree are never touched. The old push did `git checkout -f state`, which
left the job standing on a branch that holds nothing but the encrypted blob (no
.github/, so no local composite action could be loaded after it) and which could
be refused outright by one modified tracked file. Neither is possible now.

The encryption is unchanged: AES-256-CBC through openssl with PBKDF2, keyed by
STATE_ENCRYPTION_KEY, read from the environment as STATE_KEY so the passphrase
never appears in a command line or in a run log. Only the encrypted blob is ever
written to the branch, and the plaintext DB never leaves the runner.
"""
...
#: nothing else.
BLOB_PATH = "pond-nurture-bot/data/fub_automation.sqlite3.enc"
#: The decrypted DB, relative to the repository root (the workflows' cwd).
DB_PATH = "pond-nurture-bot/data/fub_automation.sqlite3"
STATE_BRANCH = "state"
...
def git(repo: Path, *args: str, check: bool = True, binary: bool = False, index: Optional[Path] = None):
...
def fetch_state(repo: Path) -> Optional[str]:
    ...
    if rc != 0:
        return None
    rc, sha = git(repo, "rev-parse", "--verify", f"refs/remotes/origin/{STATE_BRANCH}", check=False)
...
def default_baseline_path() -> Path:
    ...
    hours on 2026-08-11. RUNNER_TEMP is per-job on GitHub-hosted runners.
    """
...
def pull(repo: Path, db_path: Path, baseline_path: Path) -> int:
    ...
    if commit is None and remote_state_exists(repo) is not False:
        ...
        # there. A run that starts from an empty DB while the branch holds a full
        # one believes no lead has ever been emailed, and the cadence gates that
        # protect every lead read from these tables.
        raise StateSyncError(
...
def _commit_blob(repo: Path, enc_path: Path, parent: Optional[str], message: str) -> str:
    ...
    branch free of the repository it lives in.
    """
    _, blob = git(repo, "hash-object", "-w", "--", str(enc_path))
    with tempfile.TemporaryDirectory() as tmp:
        index = Path(tmp) / "index"
        git(repo, "update-index", "--add", "--cacheinfo", f"100644,{blob},{BLOB_PATH}", index=index)
...
def push(
    ...
    for attempt in range(1, attempts + 1):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            log(f"Encrypted state DB ({enc.stat().st_size} bytes) → {commit[:12]}")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    """`python -m fub_automation.state_sync pull|push` — used by
    ...
    parser.add_argument("--db", default=DB_PATH, help="Decrypted DB path, relative to --repo.")
    parser.add_argument(
