...
export async function runNightlyHealer(): Promise<HealerResult> {
  ...
  // ── Stage 0.6: Check if the unified Lifestyle Bot ran yesterday ────────────
  // The healer runs at 7pm CT. The bot runs at 10am CT via heartbeat.
  // We check the bot_run_log table (singular) for any run during today
  // (CT timezone). If no run found, we flag a warning.
  try {
    const db = await getDb();
    if (db) {
      // Yesterday in CT: midnight-to-midnight window
      const ctOffset = -5 * 60 * 60 * 1000; // CT = UTC-5
