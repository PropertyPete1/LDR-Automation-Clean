/**
 * lailaBot.ts — Laila's Lifestyle Bot
 * Agent: Laila Maria (FUB ID: 35)
...
const BOT_NAME = "Laila's Lifestyle Bot";
const BOT_SLUG = "laila";
const OBSERVATION_SOURCE = "laila_bot";

const AGENT_FUB_ID = 35;
const AGENT_FIRST = "Laila";
const AGENT_LAST = "Maria";
const AGENT_EMAIL = "Laila@lifestyledesignrealty.com";

export async function runLailaBot(): Promise<{
  sent: number;
  errored: number;
  skipped: number;
}> {
...
export async function sendLailaBotClockinEmail(): Promise<void> {
  let leadsQueued = 0;
  ...
  // Fetch real Power Queue count (1-20 day leads agent should personally text)
...
export async function sendLailaBotClockoffEmail(
