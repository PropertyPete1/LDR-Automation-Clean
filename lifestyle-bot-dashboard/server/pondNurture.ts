...
async function runStaleAgentReassignment(): Promise<{ reassigned: number; suppressed: number }> {
  ...
  for (const person of candidates) {
    ...
    // SOI Protection: Never reassign agent-imported leads (Sphere of Influence)
    if (isAgentImported(person)) {
      console.log(`[PondNurture] ${personName(person)} (${personId}): skipped stale reassign — agent-imported SOI lead`);
      suppressed++;
      continue;
    }
