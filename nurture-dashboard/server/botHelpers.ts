  ...
  for (const p of paths) {
    try {
      if (fs.existsSync(p)) {
        ...
        console.log(`[botHelpers] Loaded ${_sharedSuppressionTags!.length} shared suppression tags and ${_sharedExcludedSources!.length} excluded sources from ${p}`);
        return _sharedSuppressionTags!;
      }
    } catch (e) {
      console.warn(`[botHelpers] Failed to load suppression config from ${p}:`, e);
    }
  }
...
/** Get the shared excluded sources list (loaded alongside tags) */
export function getSharedExcludedSources(): string[] {
  if (_sharedExcludedSources === null) {
    getSharedSuppressionTags(); // triggers load of both
  }
...
//   1. createdById ≠ 2 (not Peter) AND createdVia == "Manually"
//   2. Any tag starting with "SOI" (case-insensitive)
//   3. Source CONTAINS "SOI" (case-insensitive)
// SOI leads are fully invisible to ALL automation.

/**
 * Check if a lead is SOI-silenced. Returns the matched rule description or null.
 */
export function isSOISilenced(person: FubPerson): string | null {
  ...
  for (const tag of tags) {
    ...
    if (tagName.startsWith("soi")) {
      return `tag starts with SOI: "${typeof tag === "string" ? tag : tag.name}"`;
    }
  }

  // Rule 1: createdById ≠ 2 AND createdVia == "Manually"
  if (person.createdById != null && person.createdById !== PETER_USER_ID && 
      (person.createdVia ?? "").toLowerCase() === "manually") {
    return `manually created by non-Peter user (createdById=${person.createdById}, createdVia=Manually)`;
  }
...
const PURCHASE_PIPELINE_IDS = new Set([1, 2]); // Buyers, Sellers
const LEASE_LISTING_PIPELINE_ID = 5;           // Residential Lease Listings
...
// In-memory cache for the current run (cleared on process restart)
const dealCache = new Map<number, FubDeal[]>();

/**
 * Fetch all deals for a person from FUB API. Cached per-run.
 */
/**
...
/**
 * FAIL-CLOSED. This used to swallow the error and cache [] — which every caller
 * reads as "this person has no deals", silently disabling the strongest
 ...
 */
export async function getPersonDeals(personId: number): Promise<FubDeal[]> {
  if (dealCache.has(personId)) {
    const cached = dealCache.get(personId)! as unknown;
    if (cached === DEAL_FETCH_FAILED) {
      // Already failed this run — don't re-hit the API once per rule.
      throw new DealCheckUnavailable(personId, DEAL_FETCH_ATTEMPTS);
    }
    return cached as FubDeal[];
  }
  let lastErr: unknown;
  for (let attempt = 1; attempt <= DEAL_FETCH_ATTEMPTS; attempt++) {
    try {
      const resp = await fubRequest<{ deals?: FubDeal[] }>(
        `/deals?personId=${personId}&limit=25`
      );
      const deals = resp.deals ?? [];
      dealCache.set(personId, deals);
      return deals;
    } catch (err) {
      lastErr = err;
      if (attempt < DEAL_FETCH_ATTEMPTS) {
        console.warn(
          `[DealCheck] deals fetch failed for person ${personId} ` +
  ...
  console.error(
    `[DealCheck] deals fetch FAILED for person ${personId} after ` +
      `${DEAL_FETCH_ATTEMPTS} attempts — failing CLOSED:`, lastErr);
  dealCache.set(personId, DEAL_FETCH_FAILED as unknown as FubDeal[]);
 ...
 * Used to block pond reassignment.
 */
 ...
 * These leads qualify for Phase 3 quarterly drip.
 */
export async function hasClosedPurchaseDeal(personId: number): Promise<boolean> {
  // Fail-closed: an unavailable deals API must not be read as 'no deals'.
  // Returning false here means not Phase-3 eligible — all three rules converge on DO NOT SEND.
  try {
    const deals = await getPersonDeals(personId);
    return deals.some(d => {
      const isClosed = d.closedStage || d.stageName?.toLowerCase() === "closed";
      return PURCHASE_PIPELINE_IDS.has(d.pipelineId) && isClosed;
    });
  } catch (err) {
    if (err instanceof DealCheckUnavailable) {
      console.warn(`[DealCheck] person ${personId}: deal check unavailable — failing CLOSED (not Phase-3 eligible)`);
      return false;
    }
    throw err;
  }
...
export async function isLeaseListingSilenced(personId: number): Promise<boolean> {
  ...
  try {
    const deals = await getPersonDeals(personId);
    let hasClosedLease = false;
    let hasClosedPurchase = false;
    for (const deal of deals) {
      const isClosed = deal.closedStage || ["closed", "lease listing - closed"].includes(deal.stageName?.toLowerCase() ?? "");
      if (deal.pipelineId === LEASE_LISTING_PIPELINE_ID && isClosed) {
        hasClosedLease = true;
      }
      if (PURCHASE_PIPELINE_IDS.has(deal.pipelineId) && isClosed) {
        hasClosedPurchase = true;
      }
    }
    // Purchase deal wins over lease listing
    if (hasClosedLease && !hasClosedPurchase) {
      console.log(`[DealCheck] Person ${personId} is LEASE LISTING SILENCED (closed lease, no purchase deal)`);
      return true;
    }
    return false;
  } catch (err) {
...
/** Clear the deal cache (call at start of each bot run) */
...
export async function shouldSkipLead(person: FubPerson): Promise<{ skip: boolean; reason?: string }> {
  ...
  const excludedSource = isExcludedSource(person);
  if (excludedSource) {
    console.log(`[skipGate] person ${person.id} skipped: excluded source: ${excludedSource}`);
    return { skip: true, reason: `excluded source: ${excludedSource}` };
  }

  // ── SOI Total Silence (cheap local check — no API call) ─────────────────────
  const soiRule = isSOISilenced(person);
  if (soiRule) {
    console.log(`[skipGate] person ${person.id} skipped: soi_silenced (rule matched: ${soiRule})`);
    return { skip: true, reason: `soi_silenced (rule matched: ${soiRule})` };
  }

  // Deal-Based Pond Protection: ANY lead with a deal in FUB is protected
  // Rule A: Any deal → skip (no automation touches deal-room leads)
  ...
  // (This is technically redundant now since hasAnyDeal catches it, but kept for explicit logging)
